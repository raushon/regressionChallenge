...
However, in this bivariate regression, we're regressing Anxiety on StressSurvey (not the true Stress variable). Since StressSurvey is a non-linear proxy for Stress, the estimated coefficient will not match the true coefficient of 1.0 for Stress. The regression is trying to fit a linear relationship between StressSurvey and Anxiety, but the underlying relationship is non-linear because:
...
The estimated coefficient from this regression represents the average change in Anxiety per unit change in StressSurvey, but this is a distorted estimate because it's trying to capture both the Stress→Anxiety relationship and the Time→Anxiety relationship through the StressSurvey variable alone.
